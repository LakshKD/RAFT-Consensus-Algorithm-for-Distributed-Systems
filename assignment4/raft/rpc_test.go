...
var node_port[] int =make([]int,6)
func TestMain(m *testing.M) {
	fmt.Println("Wait!!!!!!!!Starting the Cluster")
     var path string
     var servers_pid [6]interface{}
     path = "/home/laksh/Gowork/bin/raft"
     var nodeparam[] string=make([]string,3)
     var port = 8080
    
     procAttr := new(os.ProcAttr)
	procAttr.Files = []*os.File{os.Stdin, os.Stdout, os.Stderr}
     for i:=1;i<=5;i++{
     	fmt.Println("Starting Node ", i, "at port:", port)
     	node_port[i] = port
     	nodeparam[1] = strconv.Itoa(i)
     	nodeparam[2] = strconv.Itoa(port)
     	proc,err:=os.StartProcess(path,nodeparam,procAttr)
     	servers_pid[i] = proc
        if err!=nil {
	...
	m.Run()
	for i:=1;i<=5;i++{
		err := servers_pid[i].(*os.Process).Signal(syscall.SIGKILL)
...
func TestRPC_BasicSequential(t *testing.T) {
	...
	//fmt.Printf("Response from client handler",m)
...
func mkClientversion2(addr int) *Client {
	var client *Client
	raddr, err := net.ResolveTCPAddr("tcp", fmt.Sprintf("localhost:%d", node_port[addr]))
	if err == nil {
		conn, err := net.DialTCP("tcp", nil, raddr)
		if err == nil {
			client = &Client{conn: conn, reader: bufio.NewReader(conn)}
...
func (cl *Client) rcv() (msg *Msg, err error) {
	...
	if err == nil {
		...
		if msg.Kind == 'C' {
			...
			cl = mkClientversion2(msg.Version)
...
func parseFirst(line string) (msg *Msg, err error) {
	...
	case "ERR_REDIRECT":
		msg.Kind = 'R' 
		msg.Version,_ = strconv.Atoi(fields[1])
	default:
